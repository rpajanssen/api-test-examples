...
#    Then status 200
  ...
  Scenario: Should not update if the person data is invalid
    ...
    When method PUT
    Then status 400
    And match response.code == "BAD_REQUEST"
    And match response == { "code":"BAD_REQUEST","message":"update.arg0.firstName firstName is not allowed to be empty\n" }
